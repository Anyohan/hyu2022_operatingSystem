...
void *check_rows(void *arg)
{
    ...
    for(i = 0;i<9;i++){
        ...
        // check 배열이 정렬 되었기 때문에 1~9 까지 숫자와 i 를 비교
        for(k=0;k<9;k++){
            if(check[k] != k+1){
                valid[0][i] = 0;
                break;
...
void *check_subgrid(void *arg)
{
    ...
    //0~8번의 subgrid 시작 좌표를 args 를 통해 선언
    int start_x = (*args/3) * 3;
    int start_y = (*args%3) * 3;
    
    //args 로 받은 해당 subgrid 의 valid 값 초기화
    valid[2][*args] = 1;
    
    //서브그리드에 들어있는 숫자들을 check 배열에 저장 및 초기화
...
void check_sudoku(void)
{
    ...
    if(pthread_create(&rowtid,NULL,check_rows,NULL) != 0){
        fprintf(stderr, "pthread_create error: check_row \n");
        exit(-1);
    }
    // 여기를 완성하세요
    /*
    ...
    if(pthread_create(&coltid,NULL,check_columns,NULL) != 0){
        fprintf(stderr, "pthread_create error: check_col \n");
        exit(-1);
    }
    // 여기를 완성하세요
    /*
     ...
     */
    for(int i = 0;i<9;i++){
        // 공통된 변수를 공유하게 되면 무작위성을 해결할 수 없음.
        gid[i] = i;
